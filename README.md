...
- 보통 `[YOUR_NAME]`같은 `[]` 사이에 있는 내용은 `[]`에 해당 변수를 입력하라는 의미로 쓰인 것입니다.
  - 만약 당신의 GitHub 닉네임이 `hello`인데, `https://github.com/[YOUR_NAME]`를 입력하라는 것은 `https://github.com/hello`로 바꾸라는 의미입니다.
- 만약 `[]` 안에 두개 이상의 변수가 들어갈 수 있을 때는 보통 `[OPTION_A|OPTION_B]`의 형태가 사용됩니다.
  - 예를 들어, `git [pull|fetch]`는 `git pull` 혹은 `git fetch`라는 의미입니다.
- 리눅스를 기반으로 설명함에도 편의상 디렉토리는 폴더라고 칭하고 있습니다. 
  - 이는 폴더를 아는 사람은 디렉토리를 모를 수도 있지만, 그 반대의 경우는 없다고 가정했기 때문입니다.

## FAQ

- 윈도우인데 잘 안돼요.
  - 본 설명은 리눅스 사용을 가정하고 쓰였습니다. 데스크탑에 간단히 WSL로 우분투를 설치 하실 수 있고, 아니면 ChatGPT한테 복붙하고 윈도우용으로 바꿔달라고 하면 바꿔줍니다.
  - 또한 명령 프롬프트(CMD) 대신 Windows PowerShell을 이용하시면 좀 더 원활히 진행하실 수 있습니다.
...
- Academic ability
  - B.S., Dept. Computer Engineering
  - M.S. Course, Dept. Computer Engineering(Computer Network)
